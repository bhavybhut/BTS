...
public class ViewProjects {
	...
	public static List<Project> getFilterProject(ViewProjFilter filter)
	{
	...
	public static List<ProjectStatus> getFilterProjectStatus(ViewProjFilter filter)
	{
		List<ProjectStatus> someProjects = new ArrayList<ProjectStatus>();
		String name = filter.getName().toLowerCase();
		...
		for(Iterator<Project> i = projectList.iterator(); i.hasNext();)
		{
			Project tmp = i.next();
			if(tmp.getName().toLowerCase().contains(name))
				someProjects.add(new ProjectStatus(tmp, false));
		}
		return someProjects;
	}
}
