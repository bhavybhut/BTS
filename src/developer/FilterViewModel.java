...
public class FilterViewModel implements Composer{
	...
	private ViewProjFilter projectFilter = new ViewProjFilter();
	private List<Bug> currentBug = ViewBugs.getAllBugs();
	 List<Project> currentProject = ViewProjects.getAllProjects();
	private List<BugStatus> bugStatuses = generateBugStatusList(ViewBugs.getAllBugs());
	private List<ProjectStatus> projectStatus = generateProjectStatusList(ViewProjects.getAllProjects());
	...
	public String getBugFooter(){
		return String.format(bugFooterMessage, bugStatuses.size());
	}
	...
	public String getProjectStatusFooter(){
		return String.format(projFooterMessage, projectStatus.size());
	}
	
	public ListModel<BugStatus> getBugStatus(){
		return new ListModelList<BugStatus>(bugStatuses);
	}
	...
	public ViewProjFilter getProjectFilter(){
	...
	@Command
	@NotifyChange({"bugStatus","bugFooter"})
	public void changeBugFilter(){
		bugStatuses = ViewBugs.getFilterBug(bugFilter);
	}
	
	@Command
	@NotifyChange({"projectModel","projectFooter"})
	public void changeProjectFilter(){
		currentProject = ViewProjects.getFilterProject(projectFilter);
	}
	
	@Command
	@NotifyChange({"projectStatus","projectStatusFooter"})
	public void changeProjectSummaryFilter(){
		projectStatus = ViewProjects.getFilterProjectStatus(projectFilter);
	}
